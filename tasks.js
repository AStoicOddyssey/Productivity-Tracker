...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    let query = supabase
      .from('tasks')
      .select('*, branches(name, colour, hourly_rate)')
  ...
  if (req.method === 'POST') {
    const { title, description, branch_id, billable, due_at, hourly_rate } = req.body;
  ...
  if (req.method === 'PATCH') {
    const { id, title, description, branch_id, billable, due_at, status, sort_order, hourly_rate } = req.body;
